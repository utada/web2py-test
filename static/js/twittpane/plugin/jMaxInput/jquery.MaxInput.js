(function($) {
	...
	$.fn.limit = function(o,obj){
			if(!$('.jMax-text',obj).length){
				...
				var _jMaxsubmit	= $(document.createElement('div')).addClass('submit_tweet');
        /*
        var html = '<div id="shrink_url"><img id="shrink_url_button" title="shrink url" src="'+Config.shrink_url_image+'" width="24" heigth="24"></div>';
        $('#tweetbox').append(html);
        */
				var _jMaxinput		= $(document.createElement('input')).attr('type','submit').attr('disabled','true').addClass('disabled').val('tweet it');
